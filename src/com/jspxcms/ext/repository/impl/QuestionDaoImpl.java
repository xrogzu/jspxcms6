...
public class QuestionDaoImpl implements QuestionDaoPlus {
	public List<Question> findList(Integer userId, Integer historyUserId,
		...
		JpqlBuilder jb = predicate(userId, historyUserId, inPeriod, status);
	...
	private JpqlBuilder predicate(Integer userId, Integer historyUserId,
			Boolean inPeriod, Integer[] status) {
		JpqlBuilder jb = new JpqlBuilder(
				"select distinct bean from Question bean");
		jb.append(" where 1=1");
		if (userId != null) {
