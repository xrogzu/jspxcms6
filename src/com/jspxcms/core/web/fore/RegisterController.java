...
public class RegisterController {
	...
	public String forgotPasswordForm(HttpServletRequest request, HttpServletResponse response,
		...
		return site.getTemplate(RegisterController.FORGOT_PASSWORD_TEMPLATE);
	}

	@RequestMapping(value = "/forgot_password.jspx", method = RequestMethod.POST)
	public String forgotPasswordSubmit(String username, String email, String captcha, HttpServletRequest request,
			HttpServletResponse response, org.springframework.ui.Model modelMap) {
		Response resp = new Response(request, response, modelMap);
		String result = validateForgotPasswordSubmit(request, resp, username, email, captcha);
		if (resp.hasErrors()) {
			return result;
		}
	...
	public String registerForm(HttpServletRequest request, HttpServletResponse response,
		...
		Map<String, Object> data = modelMap.asMap();
		ForeContext.setData(data, request);
		return site.getTemplate(RegisterController.REGISTER_TEMPLATE);
	}
	...
	public String registerMessage(String email, Integer verifyMode, HttpServletRequest request,
		...
		Map<String, Object> data = modelMap.asMap();
		ForeContext.setData(data, request);
		return site.getTemplate(RegisterController.REGISTER_MESSAGE_TEMPLATE);
	}
	...
	public String registerSubmit(String captcha, String username, String password, String email, String gender,
		...
		User user = userService.register(ip, groupId, orgId, status, username, password, email, null, null, gender,
				birthDate, bio, comeFrom, qq, msn, weixin);
	...
	public String registerAdminSubmit(Integer roleId, String captcha, String username, String password, String realName,
			String email, String gender, Date birthDate, String bio, String comeFrom, String qq, String msn,
		...
		int orgId = reg.getOrgId();
		int status = verifyMode == GlobalRegister.VERIFY_MODE_NONE ? User.NORMAL : User.UNACTIVATED;
		User user = userService.registerAdmin(ip, roleId, groupId, orgId, status, username, password, realName, email,
				null, null, gender, birthDate, bio, comeFrom, qq, msn, weixin);
		if (verifyMode == GlobalRegister.VERIFY_MODE_EMAIL) {
			GlobalMail mail = site.getGlobal().getMail();
			String subject = reg.getVerifyEmailSubject();
			String text = reg.getVerifyEmailText();
	...
	public String retrievePasswordForm(String key, HttpServletRequest request, HttpServletResponse response,
			org.springframework.ui.Model modelMap) {
		Response resp = new Response(request, response, modelMap);
		...
		Site site = Context.getCurrentSite(request);
		User retrieveUser = userService.findByValidation(Constants.RETRIEVE_PASSWORD_TYPE, key);
		modelMap.addAttribute("retrieveUser", retrieveUser);
		modelMap.addAttribute("key", key);
		Map<String, Object> data = modelMap.asMap();
		ForeContext.setData(data, request);
		return site.getTemplate(RegisterController.RETRIEVE_PASSWORD_TEMPLATE);
	}
	...
	private String validateForgotPasswordSubmit(HttpServletRequest request, Response resp, String username,
		...
		if (!Validations.exist(forgotUser)) {
			return resp.post(501, "forgotPassword.usernameNotExist", new String[] { username });
		}
	...
	private String validateRegisterSubmit(HttpServletRequest request, Response resp, GlobalRegister reg, String captcha,
		...
		if (!Validations.pattern(email, ".*@iflytek.com$", messages, "email")) {
			return resp.post(408);
		}
	...
	public String verifyMember(String key, HttpServletRequest request, HttpServletResponse response,
		...
		Map<String, Object> data = modelMap.asMap();
		ForeContext.setData(data, request);
		return site.getTemplate(RegisterController.VERIFY_MEMBER_TEMPLATE);
	}
}
